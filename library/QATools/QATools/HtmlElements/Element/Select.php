...
class Select extends AbstractTypifiedElement implements ISimpleSetter
{
	...
	{
		$xpath = 'descendant-or-self::option[@value = ' . $this->getEscaper()->escapeLiteral($value) . ']';
	...
	{
		if ( $exact_match ) {
			$xpath = 'descendant-or-self::option[normalize-space(.) = ' . $this->getEscaper()->escapeLiteral($text) . ']';
		}
		else {
			$xpath = 'descendant-or-self::option[contains(., ' . $this->getEscaper()->escapeLiteral($text) . ')]';
		}
